...
<html lang="en">
  ...
  <body id="page-top">
		...
		<div class="content-form">
			<form class="form-inline" action="" method="GET" enctype="multipart/form-data" autocomplete="off">
				<div class="form-group">
					<label for="Tahun" style="padding-right:20px; padding-left:25px; margin-top:-20px">Tahun</label>
					<input type="text" class="form-control" id="tahun" name="tahun" placeholder="Masukan Tahun..." style="width:225px; margin-bottom:20px;" required>
				</div>
				<div class="form-group">
		...
		<?php if(isset($_GET['submit1'])) : ?>
			<?php
				$tahun = $_GET['tahun'];
				$sql = "SELECT * FROM eklipingCetak WHERE tahun='$tahun'";
			?>
			
			<div class="container">
				<?php if ($result = mysqli_query($link, $sql)): ?>
					<?php if(mysqli_num_rows($result) > 0): ?>
						<h2 class="font-weight-bold text-center">Rekapitulasi E-Kliping(Cetak) Tahun <?php echo $tahun; ?></h2>
						<table class="table">
					...
					<?php else : ?>
						<hr class="my-4">
						<h2 class="font-weight-bold text-center">Maaf, Tidak Ada Data Untuk Tahun <?php echo $tahun; ?> </h2>
					<?php endif; ?>
				<?php else: ?>
		...
		<?php elseif(isset($_GET['submit2'])) : ?>
			<?php
				$tahun = $_GET['tahun'];
				$sql = "SELECT * FROM eklipingOnline WHERE tahun='$tahun'";
			?>
			
			<div class="container">
				<?php if ($result = mysqli_query($link, $sql)): ?>
					<?php if(mysqli_num_rows($result) > 0): ?>
						<h2 class="font-weight-bold text-center">Rekapitulasi E-Kliping(Online) Tahun <?php echo $tahun; ?></h2>
						<table class="table">
							<thead class="thead-dark">
							<tr>
								<th scope="col">Tanggal</th>
